...
- **Acceptance criteria:** integration test seeding cross-department data confirms zero leakage. ✅ — executed against the real dev DB (not just written): seeded an org/department/project/three users (member, dept_admin, org-member-but-not-department-member "outsider"), asserted the outsider gets zero results for a published lesson a department member gets one result for, asserted `isSuperAdmin` bypasses correctly, asserted a draft lesson never surfaces regardless of asker, and asserted `canEditLesson`/`canManageLesson` permission boundaries. All scratch data cleaned up and verified gone afterward (zero leftover rows).
- **Fix (2026-09-08, reported by Johurul):** the first pass of this feature resolved Open Question 4 wrong by default — `canContributeToDepartment` (any department member, including an org-wide `guest`) was also being used to gate *publishing*, and the create endpoint honored a client-supplied `status: "published"` with no reviewer check at all. That meant any department member or guest could publish unreviewed content directly into department/org-wide visibility and chat grounding — the exact contamination risk called out in the requirements doc's own Open Question 4, just left unresolved and defaulted to the permissive answer. Fixed by splitting `canEditLesson` (author-or-reviewer while `draft`, reviewer-only once `published`) from a new `canManageLesson` (project owner or department admin only — the actual "who may publish" set) in `src/lib/lessonAccess.js`; the create endpoints now force `status: "draft"` unconditionally regardless of client input; the publish transition (`draft` → `published`) in both PATCH endpoints now explicitly requires `canManageLesson`, returning 403 with a clear message otherwise. UI (`LessonFormModal`, `LessonsPanel`) updated to only show a Publish/Unpublish action to reviewers, with a note to non-reviewers that a review step is needed. Locked in with new integration-test assertions covering the exact reported scenario (a `guest`-role, non-admin department member can author and edit their own draft, but cannot publish it or touch anyone else's lesson).
