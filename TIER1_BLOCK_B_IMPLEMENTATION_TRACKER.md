...
- **Real bug found and fixed (2026-07-29, Johurul):** searching the department panel for "remote work" against a department with a real matching document returned nothing. Root cause was in `scopeSql()` (`src/lib/vectorSearch.js`), not in the recommendation logic itself: project-scoped documents have `Document.departmentId = null` (their department lives on `Project.departmentId` instead), but `scopeSql`'s `"department"` branch only ever checked `d."departmentId" = <deptId>` directly — so *every* project-scoped document was invisible to any department-scoped query. This is a shared helper used by `orgSearch`/`orgKeywordSearch`/`orgFallbackTextSearch` (`vectorSearch.js`) and `getDecisionEvidence` (`decisionIntelligence.js`), so it silently affected department-scoped chat retrieval and decision evidence too, not just recommendations — pre-existing, not introduced by `6-B`/`6-C`. Fixed by adding an `EXISTS`-against-`Project` branch (both the explicit-`departmentId` case and the no-department-chosen membership fallback), and threading `userId` through to `scopeSql` for the fallback's correlated membership check. Verified against org `cmqln1sip0002ji7bpvnrzcyc`'s Engineering department: all 4 real documents (including both Remote Work Policy docs) now match, versus zero before the fix.
  - **Separately flagged, not yet fixed:** `src/app/api/org/[orgId]/repository/route.js` (the main "Documents" tab listing, unrelated to Phase 3) has the identical bug — its `deptFilter` also does a direct `{ departmentId: deptFilter }` match with no project-inherited fallback, so it likely returns the wrong/empty document list for any department whose documents are mostly project-scoped. Confirmed this returns zero documents for Engineering in this same org under the current code. Not fixed here since it's outside Phase 3's files — flagging for a decision on whether to fix now or file separately.
